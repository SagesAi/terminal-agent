...
class LLMClient:
    ...
    def __init__(self,
        ...
        # 记录初始化信息
        logger.info(f"Initialized LLM client with provider: {
    ...
    def chat_with_history(self,
        ...
        try:
            ...
            if system_prompt:
                messages.append(
                    self.provider.format_system_prompt(system_prompt))
            ...
            console.print(
                f"[bold red]调用 {
                    self.provider_name.upper()} API 时出错: {
                    str(e)}[/bold red]")
            # 打印更详细的错误信息以便调试
            import traceback
    ...
    def call_with_messages(
            self, messages: List[Dict[str, str]], show_progress: bool = True) -> str:
        """
            ...
            try:
                ...
                try:
                    ...
                    script_match = re.search(
                        script_pattern, json_str, re.DOTALL)
                    ...
                    if script_match:
                        ...
                        # 替换脚本内容为占位符
                        json_with_placeholder = json_str.replace(
                            script_match.group(1), placeholder)

                        # 尝试解析替换后的 JSON
                        try:
                            parsed = json.loads(json_with_placeholder)
                            if isinstance(
                                    parsed, dict) and "action" in parsed and "input" in parsed["action"]:
                                # 恢复脚本内容
